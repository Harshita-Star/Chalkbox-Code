...
<%
	...
	if (cls == null || cls.equalsIgnoreCase("") || cls.equalsIgnoreCase("0")) {
		cls = "All";
	}
		...
		try {
			...
			while (i.hasNext()) {
				...
				if (!fi.isFormField()) {
					...
					String fileName1 = "new_file_gallery_" + time + "_" + String.valueOf(randomPIN) + ".jpg";
					...
					if(!imglist.contains(fileName1))
					{
						imglist.add(fileName1);
					}
			...
			{
				if (galReq.equalsIgnoreCase("no")) 
				{
					new DataBaseMeathodJson().addGalleryImages(allfilename, gname, schid, conn, cls,"yes");
				} 
				else
				{
					new DataBaseMeathodJson().tempGallery(imglist, "new", gname, schid, userid,cls, conn);
				}
			...
			else if (type.equalsIgnoreCase("edit")) 
			{
				if (galReq.equalsIgnoreCase("no")) 
				{
					new DataBaseMeathodJson().updateGalleryImages(allfilename, gname, schid, conn, tagid, cls,"yes");
				} 
				else
				{
					new DataBaseMeathodJson().tempGallery(imglist, "old", tagid, schid, userid,cls, conn);
				}
			}
