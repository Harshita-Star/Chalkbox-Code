...
@ManagedBean(name = "studentTcModule")
@ViewScoped
public class StudentTcBean implements Serializable {
	...
	String json = "";
	String reason, lastClass, branchId, tcNumber, schid, text, previousClass, session, headerImage, website, perfom,
			classname2, dob1, startingdate1, dobinword, fullName, classname1, addmissionnumber;
	Date struckOffDate, applicationDate, issueDate, dob, startingDate;
	boolean showTextBox, showTC, showMainForm;
	String fatherName, motherName, nationality, category, date1, year, className, currentAddress, permanentAddress,
			issuDateStr, lastDateStr, tcType = "New";
	String schoolExam, failedOrNot, subjectStudied, qualifiedPromotion, monthOfFeePaid, workingDays, workingDayPresent,
	...
	public StudentTcBean() throws ParseException {
		Connection conn = DataBaseConnection.javaConnection();

		Map<String, String> params = FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap();

		tcNumber = params.get("tcNumber");
		tcIssueDate = params.get("IssueDate");
		schid = params.get("Schoolid");
		
		Date iDate = new SimpleDateFormat("dd/MM/yyyy").parse(tcIssueDate);
		String finalDate = new SimpleDateFormat("yyyy-MM-dd").format(iDate);
		
		// for data table cbsc_old_student_tc
		studentDetails = dbExam.getOldStudentDetailsForTc(tcNumber, schid, finalDate, conn);
		if (studentDetails.isEmpty() == true) {
			// for data table transfer_certificate
			studentDetails = dbExam.getStudentDetailsFOrTc(tcNumber, schid, studentDetails, finalDate, conn);
