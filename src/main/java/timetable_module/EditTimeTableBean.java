...
@ManagedBean(name="editTimeTableBean")
@ViewScoped

public class EditTimeTableBean implements Serializable
{
	...
	public EditTimeTableBean()
	{
		...
		objTT.timeTableOfAllClass(changeTableList,con);
		showTable=true;
		...
		for(TimeTableComponentInfo info:changeTableList)
		{
			if(selectedRow==info.getsNo())
			{
				if(!info.getCourseId().equals("") && !info.getSectionId().equals(""))
				{
					...
					timeTableList=showTable("time_table");
	...
	public void update()
	{
		...
		{
			int i;
			objTT.deleteTimetableOFParticular(selectedCourse,selectedSection,schoolId,con);
			i=objTT.insertTimeTable(selectedCourse,selectedSection,timeTableList,schoolId,con);
			if(i>=1)
			{
				String refNo;
				try {
	...
	public String addWorkLog(Connection conn)
	{
		...
		language ="Class-"+clasname+" --Section-"+sectonName;
		...
		String refNo = workLg.saveWorkLogMehod(language,"Edit Time Table","WEB",value,conn);
		return refNo;
	}
