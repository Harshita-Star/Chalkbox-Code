...
@ManagedBean(name="ReligionWiseReport")
@ViewScoped
public class ReligionWiseReportBean implements Serializable
{
	...
	public ReligionWiseReportBean()
	{
		...
		studentList=obj.religionWiseStudentList(religionList,/*genderList,*/conn);
		columnsList.clear();
	...
	public  void exportReligionPdf() throws DocumentException, IOException, FileNotFoundException {
		...
		try {
			...
			for (int k=0;k<studentList.size();k++){
				...
				for(int i=0;i<religionList.size();i++) {
					...
					tableq.addCell(new Phrase(studentList.get(k).getAttendanceMap().get(religionList.get(i).getValue()+"Transgender"),fontq));
...
//		file = new DefaultStreamedContent(isFromFirstData, "application/pdf","Religion_Wise_Report.pdf");
		file = new DefaultStreamedContent().builder().contentType("application/pdf").name("Religion_Wise_Report.pdf").stream(()->isFromFirstData).build();
	...
	public void toNum(Object doc)
	{
	 ...
	 sheet.createFreezePane(0,5);
	     ...
	     XSSFCell celldynamic2223 = secondRow.createCell(3*i);
	     celldynamic2223.setCellValue("Transgender");
	     }
