...
@ManagedBean(name="classFeeCollection")
@ViewScoped
public class ClassWiseFeeCollectionBean implements Serializable
{
	private static final long serialVersionUID = 1L;
	Date startDate,endDate;
	String date,schoodlId,sessionValue;
	...
	public void searchData()
	{
		...
		schoodlId=obj1.schoolId();
		sessionValue=obj1.selectedSessionDetails(schoodlId, conn);
		date=sdf.format(startDate)+" - "+sdf.format(endDate);
		classFeeList=obj1.viewFeeList1(schoolDetails.getSchid(),conn);
		FeeInfo ff=new FeeInfo();
		ff.setFeeName("Total");
		classFeeList.add(ff);

		feeList=dbr.classWiseFeeReport(startDate,endDate,classFeeList,conn);
		showData=true;
		if(feeList.size()>0)
		{
	...
	public  void exportClasFeePdf() throws DocumentException, IOException, FileNotFoundException {
		...
		try {
			...
			PdfPTable table = new PdfPTable(1+classFeeList.size());
			...
			for (int i=0;i<feeList.size();i++){
				...
				table.addCell(new Phrase(feeList.get(i).getClassName()));
...
//		file = new DefaultStreamedContent(isFromFirstData, "application/pdf","Class_Wise_Fee_Report.pdf");
		file = new DefaultStreamedContent().builder().contentType("application/pdf").name("Class_Wise_Fee_Report.pdf").stream(()->isFromFirstData).build();
	...
	public void toNumClassFee(Object doc)
	{
