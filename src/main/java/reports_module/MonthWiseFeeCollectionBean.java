...
@ManagedBean(name="monthFeeCollection")
@ViewScoped
public class MonthWiseFeeCollectionBean implements Serializable
{
	private static final long serialVersionUID = 1L;
	String startMonth,endMonth,session;
	...
	public void searchData()
	{
		...
		feeList=dbr.monthWiseFeeReport(startMonth,endMonth,classFeeList,conn);
		if(feeList.size()>0)
		{
	...
	public  void exportMonthFeePdf() throws DocumentException, IOException, FileNotFoundException {
		...
		try {
			...
			for (int i=0;i<feeList.size();i++){
				...
				table.addCell(new Phrase(feeList.get(i).getMonthName()));
...
//		file = new DefaultStreamedContent(isFromFirstData, "application/pdf","Month_Wise_Fee_Report.pdf");
		file = new DefaultStreamedContent().builder().contentType("application/pdf").name("Month_Wise_Fee_Report.pdf").stream(()->isFromFirstData).build();
	...
	public void toNumMonthFee(Object doc)
	{
