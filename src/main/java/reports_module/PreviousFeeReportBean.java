...
@ManagedBean(name="preFeeReport")
@ViewScoped
public class PreviousFeeReportBean implements Serializable
{
	...
	String session,schoolId,name,selectedCLassSection,selectedSection;
	DatabaseMethods1 dbm=new DatabaseMethods1();


	public PreviousFeeReportBean()
	{
	...
	public void searchStudentByName()
	{
		...
		if(index!=0)
		{
					try
					{
						studentList=obj.getPreviousFeeData(searchBasis,"","",schoolId,id,conn);
	...
	{
		String searchBasis="byclass";
		Connection conn = DataBaseConnection.javaConnection();
		//studentList=obj.allPreviousFeeList(schoolId,conn);
		
		try
		{
			studentList=obj.getPreviousFeeData(searchBasis,selectedCLassSection,selectedSection,schoolId,"",conn);
			if(studentList.size()==0)
			{
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("No record Found"));
				b=false;
	...
	public void updateFeeDetails()
	{
		...
		{
			int i=dbm.updatePreviousFees(selectedStudent.getAddNumber(),fees,session,conn);
			if(i==1)
			{
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("Fee Updated Sucessfully"));
				studentList=new DataBaseMethodsReports().allPreviousFeeList(schoolId,conn);
			}
			else
			{
	...
	{
		Connection conn=DataBaseConnection.javaConnection();
		int i=dbm.deletePreviousFees(selectedStudent.getAddNumber(),conn);
		if(i==1)
		{
