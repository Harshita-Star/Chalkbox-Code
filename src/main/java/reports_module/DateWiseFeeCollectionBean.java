...
@ManagedBean(name="dateFeeCollection")
@ViewScoped
public class DateWiseFeeCollectionBean implements Serializable
{
	...
	public void searchData()
	{
		...
		feeList=dbr.dateWiseFeeReport(startDate,endDate,classFeeList,conn);
		if(feeList.size()>0)
		{
	...
	public  void exportDateWiseColPdf() throws DocumentException, IOException, FileNotFoundException {
		...
		try {
			...
			for(int i=0;i<classFeeList.size();i++) {
				table.addCell(new Phrase(String.valueOf(classFeeList.get(i).getFeeName()),font));
			}
			...
			for (int i=0;i<feeList.size();i++){
				...
				table.addCell(new Phrase(String.valueOf(feeList.get(i).getDateName()),font));
...
//		file = new DefaultStreamedContent(isFromFirstData, "application/pdf","Date_Wise_Collection_Report.pdf");
		file = new DefaultStreamedContent().builder().contentType("application/pdf").name("Date_Wise_Collection_Report.pdf").stream(()->isFromFirstData).build();
	...
	public void toNumDate(Object doc)
	{
		...
		//   clee.setCellValue("Date Period Ext.:-");
