...
@ManagedBean(name="viewAdminParentJson")
@ViewScoped
public class ViewAdminParentJson implements Serializable{

	String json;
	ArrayList<MessageInfo> list;
	DataBaseMeathodJson DBJ=new DataBaseMeathodJson();

	public ViewAdminParentJson()
	{
		...
		try {
			...
			if(checkRequest)
			{
				list=DBJ.viewInnerBroadcastMessageIndivisual(schid,groupid,studentid,conn);
				if(list.size()>0)
				{
