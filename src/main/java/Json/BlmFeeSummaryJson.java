...
@ManagedBean(name="blmFeeSummaryJson")
@ViewScoped
public class BlmFeeSummaryJson implements Serializable {
	...
	public BlmFeeSummaryJson()
	{
		...
		try 
		{
			...
			if(checkRequest)
			{
				...
				for(int i=0;i<feemonths.length;i++)
				{
					...
					for (FeeInfo ls : classFeeList) {
						if (!ls.getFeeId().equals("-2") && !ls.getFeeId().equals("-3") && !ls.getFeeId().equals("-4")) {
							...
							feeAmount=feeAmount+mainamount;
				...
				ss.put("total_fee",String.valueOf(feeAmount));
				ss.put("total_paid",String.valueOf(paidAmount));
