...
@ManagedBean(name="monthwiseHomeWorkJson")
@ViewScoped 
public class MonthWiseHomeWorkJson implements Serializable {
		...
		try {
			...
			if(checkRequest)
			{
				if(type==null)
				{
					...
					list=DBJ.viewAssignmentjsonForMonth(selectedSection,month,schid,conn,selectedCLassSection,year);
				}
				else
				{
					...
					{
						list=DBJ.viewAssignmentjsonForMonth("",month,schid,conn,"",year);
					}
					else
					{
						selectedStudent=DBJ.studentDetailslistByAddNo(studentid, schid,conn);
