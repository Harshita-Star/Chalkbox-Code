...
@ManagedBean(name="viewOnlineLactureJson")
@ViewScoped
public class ViewOnlineLactureJson implements Serializable {
		...
		try {
			...
			if(checkRequest)
			{
				if(type==null)
				{
					...
					list=DBJ.viewOnlineLacturejson(selectedSection,newDate,schid,conn,selectedCLassSection,offcet,count,off);
				}
				else
				{
					...
					{
						list=DBJ.viewOnlineLacturejson("",newDate,schid,conn,"",offcet,count,off);
					}
					else
					{
						selectedStudent=DBJ.studentDetailslistByAddNo(studentid, schid,conn);
