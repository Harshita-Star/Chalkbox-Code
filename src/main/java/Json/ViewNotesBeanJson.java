...
@ManagedBean(name="viewNotesBean")
@ViewScoped
public class ViewNotesBeanJson implements Serializable
{
		...
		try {
			...
			if(checkRequest)
			{
				if(type==null)
				{
					...
					list=DBJ.viewAssignmentjsonNotes(selectedSection,newDate,schid,conn,selectedCLassSection,offcet,count,off);
				}
				else
				{
					...
					{
						list=DBJ.viewAssignmentjsonNotes("",newDate,schid,conn,"",offcet,count,off);
					}
					else
					{
						selectedStudent=DBJ.studentDetailslistByAddNo(studentid, schid,conn);
