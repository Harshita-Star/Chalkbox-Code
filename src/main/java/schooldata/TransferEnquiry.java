...
@ManagedBean(name="transferEnquiry")
@ViewScoped
public class TransferEnquiry implements Serializable
{
	...
	{
		Connection conn=DataBaseConnection.javaConnection();
		list=new DatabaseMethods1().allEnquiryList("All", selectedSourceEmployee, conn);
	...
	public void transferLead()
	{
		...
		if(selectedList.size()>0)
		{
			...
			{
				obj.addFollowUp(info.getId(),date,"Lead Transfered","Accepted", userId,"","view", conn, DatabaseMethods1.selectedSessionDetails(new DatabaseMethods1().schoolId(),conn));
			}

			int i=obj.transferEnquiry(selectedSourceEmployee,selectedTargetEmployee,selectedList,conn);
			if(i>=1)
			{
