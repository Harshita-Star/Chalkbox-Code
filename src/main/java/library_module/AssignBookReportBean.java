...
@ManagedBean(name="assignBookReport")
@ViewScoped
public class AssignBookReportBean implements Serializable
{
	...
	public void searchData()
	{
		...
		enDate = new SimpleDateFormat("dd/MM/yyyy").format(endDate);
		bookList=objLibrary.assignBookReport(schoolId,"report",startDate,endDate,conn);
		totalRet=totalPend=0;
		if(bookList.size()>0)
		{
			for(BookInfo bb : bookList)
			{
				if(bb.getReceivedStatus().equalsIgnoreCase("yes"))
				{
					totalRet = totalRet + 1;
				}
				else if(bb.getReceivedStatus().equalsIgnoreCase("no"))
				{
	...
	public  void exportAssignBookPdf() throws DocumentException, IOException, FileNotFoundException {
		...
		try {
			...
			for (int i=0;i<bookList.size();i++){
				...
				table.addCell(new Phrase(bookList.get(i).getReceiveDateStr(),font));
...
//		file = new DefaultStreamedContent(isFromFirstData, "application/pdf","Assign_Book_Report.pdf");
		file = new DefaultStreamedContent().builder().contentType("application/pdf").name("Assign_Book_Report.pdf").stream(()->isFromFirstData).build();
	...
	public void setTotalPend(int totalPend) {
