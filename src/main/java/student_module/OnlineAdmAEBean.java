...
@ManagedBean(name="onlineAdmae")
@ViewScoped

public class OnlineAdmAEBean implements Serializable
{
	...
	public void submit()
	{
		...
		if(i>=1)
		{
			...
			DBO.updateDocInfo(info.getDocinfo(), linfo.getUnm(), enqid, conn);
			DBO.updateMedicalInfo(info.getMedinfo(), linfo.getUnm(), enqid, conn);
			DBO.updateVaccineInfo(info.getVacinfo(), linfo.getUnm(), enqid, conn);
			DBO.deleteSiblingAe(enqid, conn);
			DBO.insertSiblingInfo(completeList, enqid, linfo.getUnm(), info.getStudent_id(), conn);

			LoginInfo ll= DBO.signUpInfo(info.getLogin_id(), conn);
			String schnm = DBM.schoolNameById(schoolId, conn);


			if(info.getStatus().equalsIgnoreCase("Review Admission Form"))
			{
				subject = schnm+" Amended Admission Form Submitted!";
				heading = "<center>Hello "+info.getSt_name()+",</center>" + "<center class=\"red\">Congratulations, your amended admission form in  "+schnm+" has been submitted successfully!</center>";
			}
			else
			{
				subject = schnm+" Admission Form Submitted!";
				heading = "<center>Hello "+info.getSt_name()+",</center>" + "<center class=\"red\">Congratulations, your admission form in  "+schnm+" has been submitted successfully!</center>";
			}
			String msg = "<center><strong>Please login to your account, go to 'My Applications' under 'For Applicant' section, to check your admission form and its status.</strong><br></br>"+"<a href=\"http://chalkboxerp.in/DM/onlineAdmLogin.xhtml\"> <img style=\"height: 44px;\" src=\"http://chalkboxerp.in/loginNowButton.png\"> </a> <br></br></center>";
			...
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("Admission Form Submitted Successfully!"));
			info = DBO.onlineAdmInfoById(enqid, "id", conn);
		}
